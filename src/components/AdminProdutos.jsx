...
import { produtos as locais } from '../data/produtos';
  ...
  async function alternar(produto) {
    ...
    try {
      const res = await apiAutenticada('/api/produto-ativo', {
        ...
        body: JSON.stringify({ id: produto.id, ativo: produto.ativo === false }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Falha ao alterar produto.');
      setProdutos(lista => lista.map(p => p.id === data.id ? { ...p, ativo: data.ativo } : p));
    } catch (e) { setErro(e.message); }
    finally { setOcupado(false); }
  }
  return <section className="mb-6 rounded-lg border border-zinc-800 p-4">
    ...
    {aberto && <div className="mt-4">
      ...
      <ul className="max-h-96 overflow-y-auto divide-y divide-zinc-800">
        {produtos.map(p => {
          const bloqueado = locais.find(l => String(l.id) === String(p.id))?.ativo === false;
          return <li key={p.id} className="flex items-center justify-between gap-3 py-3">
            <div className="text-sm text-zinc-200">#{p.id} — {p.nome}
              <p className="text-xs text-zinc-500">{bloqueado ? 'Desativado no código: altere ativo para true e publique.' : p.ativo === false ? 'Desativado' : 'Ativo'}</p>
            </div>
            <button type="button" disabled={ocupado || bloqueado} onClick={() => alternar(p)} className="rounded border border-zinc-700 px-3 py-2 text-sm text-sand-400 disabled:opacity-40 focus-visible:outline focus-visible:outline-2">
